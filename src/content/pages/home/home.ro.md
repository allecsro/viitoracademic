---
pageType: home
lang: ro
title: VIITOR ACADEMIC - Sprijinim tinerii să își atingă potențialul academic
description: Sprijinim tinerii să își atingă potențialul academic
heroTitle: VIITOR ACADEMIC
heroSubtitle: Sprijinim tinerii să își atingă potențialul academic
heroImages:
  - /Images/Hero%20section/HS-1.webp
  - /Images/Hero%20section/HS-2.webp
  - /Images/Hero%20section/HS-3.webp
heroButton1Text: Află mai multe
heroButton1Link: '#about'
heroButton2Text: Implică-te
heroButton2Link: '#get-involved'
aboutTitle: Despre noi
aboutContent: >-
  Asociația VIITOR ACADEMIC este o inițiativă non-profit dedicată sprijinirii
  tinerilor talentați sau cu rezultate academice deosebite, care provin din
  familii cu resurse financiare limitate. Ne adresăm în mod special elevilor și
  studenților din mediile rurale și urbane defavorizate, oferindu-le sprijin
  moral și financiar pentru a-și continua studiile superioare. Credem în
  potențialul fiecărui tânăr și lucrăm pentru a-l transforma în oportunități
  reale de dezvoltare academică și profesională.
aboutImage: /Images/About-Mission-Vison%20Section/AS.webp
missionTitle: Misiune
missionContent: >-
  VIITOR ACADEMIC își propune să elimine barierele care limitează accesul la
  educație superioară. Prin programe educaționale, consiliere, burse și sprijin
  activ, ajutăm tinerii talentați să își descopere vocația, să își continue
  parcursul educațional și să se integreze cu succes pe piața muncii.
missionImage: /Images/About-Mission-Vison%20Section/MS.webp
visionTitle: Viziune
visionContent: >-
  Visăm la o societate în care fiecare tânăr, indiferent de mediul din care
  provine, are șansa de a studia, de a visa și de a se dezvolta. Credem într-o
  educație echitabilă, accesibilă tuturor, și într-un viitor academic în care
  talentul, nu situația socială, trasează drumul unui student.
visionImage: /Images/About-Mission-Vison%20Section/VS.webp
projectsTitle: Proiectele noastre
projectsSubtitle: Programe gândite pentru a susține și împuternici tinerii
projects:
  - title: Consiliere vocațională
    description: >-
      Activități de autocunoaștere, testare vocațională și ghidare în alegerea
      unei cariere. Îi ajutăm pe tineri să își descopere punctele forte și
      interesele pentru a lua decizii informate despre viitorul lor.
  - title: Campanii de conștientizare
    description: >-
      Promovăm nevoile tinerilor din medii vulnerabile, creștem gradul de
      conștientizare cu privire la oportunitățile educaționale și eliminăm
      barierele de acces.
  - title: Evenimente educaționale
    description: >-
      Vizite la universități, ateliere, webinarii și evenimente artistice care
      inspiră și informează tinerii despre diverse parcursuri academice și
      profesionale.
  - title: Burse și sprijin financiar
    description: >-
      Stimulente și asistență financiară pentru studenții cu rezultate academice
      excelente, astfel încât lipsurile materiale să nu le limiteze potențialul.
initiativesTitle: Inițiativele noastre
initiativesSubtitle: >-
  Descoperă inițiativele prin care susținem și împuternicim tinerii din diverse
  comunități și regiuni.
initiatives:
  - title: Atelier - "Importanța educației"
    location: '"Liceul Tehnologic Liviu Rebreanu" din Mozăceni, Argeș'
    date: 05 Decembrie 2025
    imageUrl: /Images/Initiatives%20section/IS-1.webp
  - title: Acordare Burse "Viitor Academic"
    location: '"Liceul Tehnologic Liviu Rebreanu" din Mozăceni, Argeș'
    date: 15 Decembrie 2025
    imageUrl: /Images/Initiatives%20section/IS-2.webp
stats:
  - number: 6
    label: Tineri sprijiniți
  - number: 11
    label: Burse acordate
  - number: 1
    label: Evenimente educaționale
  - number: 1
    label: Instituții partenere
statsBackgroundImage: /Images/Stats%20Section/SS-1.webp
getInvolvedTitle: Vrei să ne ajuți?
getInvolvedSubtitle: >-
  Schimbarea începe cu fiecare dintre noi. Te poți implica în inițiativa VIITOR
  ACADEMIC prin:
getInvolvedWaysTitle: Modalități de implicare
getInvolvedWays:
  - 'Voluntariat (educațional, logistic sau comunicare)'
  - Sponsorizări și donații
  - Parteneriate educaționale
  - Promovarea proiectelor noastre în comunitatea și rețeaua ta
getInvolvedMessage: >-
  Scrie-ne și haide să construim împreună un viitor academic mai echitabil prin
  VIITOR ACADEMIC!
contactUsTitle: Contact
contactEmail: mireamarian@gmail.com
contactPhone: 0721014857
donationButtonText: Descarcă formularul de donație
donationButtonLink: /Forms/RO Donation form.pdf
---

