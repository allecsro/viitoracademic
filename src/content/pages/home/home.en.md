---
pageType: home
lang: en
title: VIITOR ACADEMIC - We support young people to reach their academic potential
description: We support young people to reach their academic potential
heroTitle: VIITOR ACADEMIC
heroSubtitle: We support young people to reach their academic potential
heroImages:
  - /Images/Hero%20section/HS-1.webp
  - /Images/Hero%20section/HS-2.webp
  - /Images/Hero%20section/HS-3.webp
heroButton1Text: Learn More
heroButton1Link: '#about'
heroButton2Text: Get Involved
heroButton2Link: '#get-involved'
aboutTitle: About Us
aboutContent: >-
  The VIITOR ACADEMIC Association is a non-profit initiative dedicated to
  supporting talented young people or those with outstanding academic results,
  coming from families with limited financial resources. We specifically address
  students from disadvantaged rural and urban areas, offering them moral and
  financial support to continue their higher education. We believe in the
  potential of every young person and work to transform it into real
  opportunities for academic and professional development.
aboutImage: /Images/About-Mission-Vison%20Section/AS.webp
missionTitle: Mission
missionContent: >-
  VIITOR ACADEMIC aims to eliminate barriers that limit access to higher
  education. Through educational programs, counseling, scholarships, and active
  support, we help talented young people discover their vocation, continue their
  educational path, and successfully integrate into the labor market.
missionImage: /Images/About-Mission-Vison%20Section/MS.webp
visionTitle: Vision
visionContent: >-
  We dream of a society where every young person, regardless of their
  background, has the chance to study, dream, and develop. We believe in
  equitable education, accessible to all, and in an academic future where
  talent, not social situation, defines a student's path.
visionImage: /Images/About-Mission-Vison%20Section/VS.webp
projectsTitle: Our Projects
projectsSubtitle: Comprehensive programs designed to support and empower young students
projects:
  - title: Professional Guidance
    description: >-
      Self-knowledge activities, vocational testing, and guidance in choosing a
      career path. We help students discover their strengths and interests to
      make informed decisions about their future.
  - title: Awareness Campaigns
    description: >-
      We promote the needs of young people from vulnerable backgrounds, raising
      awareness about educational opportunities and breaking down barriers to
      access.
  - title: Educational Events
    description: >-
      University visits, workshops, webinars, and artistic events that inspire
      and educate students about various academic and career paths.
  - title: Scholarships and Financial Support
    description: >-
      Incentives and financial assistance for students with excellent academic
      results, ensuring that financial constraints do not limit their potential.
initiativesTitle: Our Initiatives
initiativesSubtitle: >-
  Discover the impactful initiatives we've launched to support and empower
  students across different communities and regions.
initiatives:
  - title: Workshop - "The Importance of Education"
    location: '"Liceul Tehnologic Liviu Rebreanu" from Mozaceni, Arges'
    date: 'December 05, 2025'
    imageUrl: /Images/Initiatives%20section/IS-1.webp
  - title: >-
      Granting of "Viitor Academic" scholarship for academic result to 11
      students
    location: '"Liceul Tehnologic Liviu Rebreanu" din Mozăceni, Argeș'
    date: 'December 15, 2025'
    imageUrl: /Images/Initiatives%20section/IS-2.webp
stats:
  - number: 6
    label: Students Supported
  - number: 11
    label: Scholarships Awarded
  - number: 1
    label: Educational Events
  - number: 1
    label: Partner Institutions
statsBackgroundImage: /Images/Stats%20Section/SS-1.webp
getInvolvedTitle: Do you want to help us?
getInvolvedSubtitle: >-
  Change starts with each of us. You can get involved in the VIITOR ACADEMIC
  initiative through:
getInvolvedWaysTitle: Ways to Get Involved
getInvolvedWays:
  - 'Volunteering (educational, logistical, or communication)'
  - Sponsorships and donations
  - Educational partnerships
  - Promoting our projects in your community and network
getInvolvedMessage: >-
  Write to us and let's build a more equitable academic future together with
  VIITOR ACADEMIC!
contactUsTitle: Contact
contactEmail: mireamarian@gmail.com
contactPhone: 0721014857
donationButtonText: Download Donation Form
donationButtonLink: /Forms/RO Donation form.pdf
---

